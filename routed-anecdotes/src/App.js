...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    props.notify(`New anecdote "${content.value}" created!`)
    navigate('/anecdotes')
  }
...
const App = () => {
  ...
  const [notification, setNotification] = useState({ text: '', timeout: null })
  ...
  const notify = (text) => {
    setNotification(current => {
      ...
      return {
        text,
        timeout: setTimeout(() => {
          setNotification({ text: '', timeout: null })
        }, 5000)
      }
    })
  }
  ...
  return (
    <div>
      <h1>Software anecdotes</h1>
        <Menu />
        {notification.text ? <Notification text = {notification.text} /> : null}
        <Routes>
